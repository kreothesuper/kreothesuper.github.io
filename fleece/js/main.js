        ...
        document.addEventListener('DOMContentLoaded', function () {
          ...
          if (timerArray.length) {
            timerArray.forEach(element => {
              ...
              const canvas = element.querySelector('.calculator-circle__border');

              console.log(canvas)

              const elementH = element.querySelector('.js-timer-hours'),
                elementD = element.querySelector('.js-timer-days'),
                elementM = element.querySelector('.js-timer-minutes'),
                elementS = element.querySelector('.js-timer-seconds');
              ...
              var x = setInterval(function () {
                ...
                elementH.textContent = hours < 10 ? `0${hours}` : hours;
                elementD.textContent = days < 10 ? `0${days}` : days;
                elementM.textContent = minutes < 10 ? `0${minutes}` : minutes;
                elementS.textContent = seconds < 10 ? `0${seconds}` : seconds;

                // var totalTime = countDownDate - now;
                // var elapsedTime = totalTime - distance;
                // var percentageRemaining = (elapsedTime / totalTime) * 100;

                percent = days / 30 * 100;

                let successValue = (percent/100)*880;
                canvas.getElementById('success-value').setAttribute('stroke-dasharray', `${successValue}, 880`);
                canvas.getElementById('success-value').setAttribute('r', `${canvas.getBoundingClientRect().width/2 - 8}`);
                if (distance < 0) {
                  clearInterval(x);
