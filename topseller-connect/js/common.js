var total, queries, stocks, labels;



function number_to(id, res) {
    var element = document.getElementById(id);
    console.log(res);
    element.setAttribute("data-end", res);
    // // COUNTER SETTINGS
...
$.ajax({
    dataType: 'json',
    type : 'post',
    url: "counter.php", //Путь к обработчику
    response: 'json'
    ...
    stocks = data.stocks;
    labels = data.labels;
    number_to("total_cnt",total);
    number_to("queries_cnt",queries);
    number_to("stocks_cnt",stocks*185);
    number_to("labels_cnt",labels*126);
})
...
const expandedList = (block, height) => {
    const expandedBlock = block.querySelector('.expanded-block'),
        expandedBlockHeight = block.clientHeight;

    if (expandedBlockHeight > height) {
        expandedBlock.style.height = `${height}px`;
        ...
        expandedBlock.dataset.heightNormal = expandedBlockHeight;

        if (expandedBlock.closest('.expanded-block-list') !== null) {
            link.addEventListener('click', () => {
                const expandedBlockList = block.closest('.expanded-block-list'),
                    expandedBlockLinks = expandedBlockList.querySelectorAll('.expanded-link'),
                    expandedBlockItems = expandedBlockList.querySelectorAll('.expanded-block');

                expandedBlockLinks.forEach(element => {
                    element.classList.toggle('active');
                });

                if (link.classList.contains('active')) {
                    expandedBlockItems.forEach(element => {
                        element.style.height = `${element.dataset.heightNormal}px`;
                    })
                } else {
                    expandedBlockItems.forEach(element => {
            ...
            link.addEventListener('click', () => {
                ...
                link.classList.contains('active') ? expandedBlock.style.height = `${expandedBlockHeight}px` : expandedBlock.style.height = `${height}px`;
            });
        }
    }
}
